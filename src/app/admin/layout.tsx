...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 bottom-0 w-64 bg-white border-r border-[#eaeaea] z-50 transform transition-transform duration-300 print:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
      ...
      {/* Main Content */}
      <main className="flex-1 lg:ml-64 print:lg:ml-0 print:ml-0 flex flex-col min-h-screen min-w-0">
        <header className="h-16 bg-white border-b border-[#eaeaea] flex items-center px-4 sm:px-6 sticky top-0 z-30 print:hidden">
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8 print:p-0 min-w-0 overflow-x-hidden">
          {children}
        </div>
      </main>
    </div>
